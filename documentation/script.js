window.devTools_connectListener = function () {
	...
	const allTryButtons = document.getElementsByClassName("devTools_try");
	for (let i = 0; i < allTryButtons.length; i++) {
...
const allDocuments = [];
const documentElements = document.getElementsByClassName("docPage");
for (var i = 0; i < documentElements.length; i++) {
	...
	const headerElements =
		documentElements[i].getElementsByClassName("docHeader");
	for (var j = 0; j < headerElements.length; j++) {
		allDocuments[documentElements[i].id].contents[headerElements[j].id] = [
			headerElements[j].innerText,
			headerElements[j].getAttribute("data-search-terms"),
		];
		allDocuments[documentElements[i].id].numOfContents++;
	}
}
let tempContentsText = "";
for (var i in allDocuments) {
	if (allDocuments[i].searchTerms) {
		tempContentsText +=
			'<li class="nav_doc_li cursorPointer" data-target-doc="' +
			i +
			'" data-search-terms="' +
			allDocuments[i].searchTerms +
			'">' +
			allDocuments[i].name +
			"</li>";
	} else {
	...
	if (allDocuments[i].numOfContents) {
		...
		for (var j in allDocuments[i].contents) {
			if (allDocuments[i].contents[j][1]) {
				tempContentsText +=
					'<li class="nav_header_li cursorPointer" data-target-doc="' +
					i +
					'" data-target-header="' +
					j +
					'" data-search-terms="' +
					allDocuments[i].contents[j][1] +
					'">' +
					allDocuments[i].contents[j][0] +
					"</li>";
			} else {
				tempContentsText +=
					'<li class="nav_header_li cursorPointer" data-target-doc="' +
...
const allItems = document.getElementById("mainList").getElementsByTagName("li");

function selectDocument(e) {
	if (e.target.getAttribute("data-target-doc")) {
		if (e.target.getAttribute("data-target-header")) {
			presentDocument(
				e.target.getAttribute("data-target-doc"),
				e.target.getAttribute("data-target-header")
			);
		} else {
...
function presentDocument(docID, headerID) {
	...
	if (headerID) {
		requestAnimationFrame(() => {
			document.getElementById(docID).scrollTop =
...
function search(keyword) {
	...
	const allDocs = document.getElementsByClassName("nav_doc_li");
	const allHeaders = document.getElementsByClassName("nav_header_li");
	for (var i = 0; i < allDocs.length; i++) {
		if (
			allDocs[i].innerText.toLowerCase().indexOf(keyword.toLowerCase()) > -1 ||
			allDocs[i]
				.getAttribute("data-target-doc")
				.toLowerCase()
	...
	for (var i = 0; i < allHeaders.length; i++) {
		if (
			allHeaders[i].innerText.toLowerCase().indexOf(keyword.toLowerCase()) >
				-1 ||
			allHeaders[i]
				.getAttribute("data-target-doc")
				.toLowerCase()
				.indexOf(keyword.toLowerCase()) > -1 ||
			allHeaders[i]
				.getAttribute("data-target-header")
				.toLowerCase()
				.indexOf(keyword.toLowerCase()) > -1
		) {
