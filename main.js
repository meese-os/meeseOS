...
const artificialLoadingScreen = true;
let currentQuip = 0;
const timePerQuip = 1500;
if (artificialLoadingScreen) {
	const audio = new Audio("./dial-up-modem.mp3");
	window.addEventListener("load", function () {
		audio.play();

		// Modified from https://stackoverflow.com/a/26869192/6456163
		const fadeLength = 250;
		const fadePoint = (timePerQuip * quips.length - fadeLength) / 1000;
		const fadeAmount = 0.1;
		const volumeReductionInterval = fadeLength / 10;
