...
describe("task > make:*", () => {
	...
	const options = utils.resolveOptions(defaults, {
		discover: [path.resolve(__dirname, "../../__mocks__/packages")],
	});
	...
	describe("make:application", () => {
		test("should create application", async () => {
			...
			expect(
				fs.readdirSync(fname("src/packages/StandardApplication"))
			).toHaveLength(7);
		});
	});
	...
	describe("make:iframe-application", () => {
		test("should create application", async () => {
			...
			expect(
				fs.readdirSync(fname("src/packages/IframeApplication"))
			).toHaveLength(6);
		});
	});
});
