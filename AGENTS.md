...
- `rushx deploy` inside `website/` assembles the production bundle once the repo is built.
...
Keep local `.env` files derived from templates under their original directories (`apps/old-site`, `apps/terminal/scripts`, `website/src`). Run `pm2 monit` only after verifying builds, and avoid exposing the development server without HTTPS or authentication. Use Node.js 22.x as pinned in `rush.json` and `.nvmrc`, and prefer `pnpm` 10.x when working outside Rush.
