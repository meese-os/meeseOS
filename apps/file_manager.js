	...
	apps.files = new Application({
		...
		vars: {
			...
			mkdir: function () {
				if (this.currLoc === "/") {
					apps.prompt.vars.alert(
						"Please navigate to a directory to create a new folder.",
						"Okay",
						function () {},
						"File Manager"
					);
					...
					apps.prompt.vars.prompt(
						"Enter a name for the new folder.<br><br>Folder will be created in " +
							this.currLoc +
							"<br><br>Leave blank to cancel.",
						"Submit",
						function (str) {
							if (str) {
								apps.savemaster.vars.mkdir(
									this.currLoc.substring(11, this.currLoc.length) + str
								);
							}
							this.update();
						}.bind(this),
			...
			mkfile: function () {
				if (this.currLoc === "/") {
					apps.prompt.vars.alert(
						"Please navigate to a directory to create a new file.",
						"Okay",
						function () {},
						"File Manager"
					);
					...
					apps.prompt.vars.prompt(
						"Enter a name for the new file.<br><br>File will be created in " +
							this.currLoc +
							"<br><br>Leave blank to cancel.",
						"Submit",
						function (str) {
			...
			deleteItem: function (itemPath) {
				apps.prompt.vars.confirm(
					"Are you sure you want to delete this item?<br><br>" +
						itemPath +
						"<br><br>WARNING: Deleting important files can cause major issues!",
					["No, keep", "Yes, delete"],
					(btn) => {
						if (btn === 1) {
			...
			deleteItemUF: function (itemPath) {
				apps.prompt.vars.confirm(
					"Are you sure you want to delete this item?<br><br>" +
						itemPath +
						"<br><br>WARNING: This cannot be undone!",
					["No, keep", "Yes, delete"],
					(btn) => {
						if (btn === 1) {
			...
			testingFolder: {
				filenameTests: {
					"This is a really long file name that has spaces and stuff in it!":
						"Hello World",
					ThisIsAReallyLongFileNameThatDoesNotHaveAnySpacesAndStuffInIt:
						"HelloWorld",
			...
			update: function () {
				...
				if (this.currLoc === "/") {
					...
					if (this.currDirList.length === 1 && this.currDirList[0] === "") {
						if (
							typeof apps.bash.vars.getRealDir(this.currLoc) !== "object" ||
							apps.bash.vars.getRealDir(this.currLoc) === null
						) {
							apps.prompt.vars.alert(
								"Could not open " +
									this.currLoc +
						...
						if (this.currLoc.indexOf("/USERFILES/") === 0) {
							for (const item in this.currDirList) {
								if (this.currDirList[item]) {
									// if item is a folder
									if (
										this.currDirList[item][
											this.currDirList[item].length - 1
										] === "/" &&
										this.currDirList[item][
											this.currDirList[item].length - 2
										] !== "\\"
									) {
										temphtml +=
											'<div class="cursorPointer" onclick="apps.files.vars.next(\'' +
											this.currDirList[item] +
											"')\" oncontextmenu=\"ctxMenu([[event.pageX, event.pageY, 'ctxMenu/file.png', 'ctxMenu/x.png'], ' Properties', 'apps.properties.main(\\'openFile\\', \\'" +
											(this.currLoc + this.currDirList[item]) +
											"\\');toTop(apps.properties)', '+Delete', 'apps.files.vars.deleteItemUF(\\'" +
											(this.currLoc + this.currDirList[item]) +
											"\\')'])\">" +
											'<img src="files/small/folder.png"> ' +
										...
										temphtml +=
											'<div class="cursorPointer" onClick="apps.notepad.vars.openFile(\'' +
											(this.currLoc + this.currDirList[item])
												.split("\\")
												.join("\\\\") +
											"');\" oncontextmenu=\"ctxMenu([[event.pageX, event.pageY, 'ctxMenu/file.png', 'ctxMenu/x.png'], ' Properties', 'apps.properties.main(\\'openFile\\', \\'" +
											...
											'<img src="files/small/' +
											this.icontype(
												typeof apps.bash.vars.getRealDir(
													this.currLoc + this.currDirList[item]
												)
											) +
											'.png"> ' +
								...
								if (this.currDirList[item]) {
									...
									if (
										...
										temphtml +=
											'<div class="cursorPointer" onclick="apps.files.vars.next(\'' +
											this.currDirList[item] +
											"')\" oncontextmenu=\"ctxMenu([[event.pageX, event.pageY, 'ctxMenu/window.png', 'ctxMenu/file.png', 'ctxMenu/x.png'], ' Open App', 'openapp(apps." +
											this.currDirList[item].substring(
												0,
												this.currDirList[item].length - 1
											) +
											", \\'dsktp\\')', '+Properties', 'apps.properties.main(\\'openFile\\', \\'" +
											(this.currLoc + this.currDirList[item]) +
											"\\');toTop(apps.properties)', '_Delete', ''])\">" +
											buildSmartIcon(
												16,
												apps[this.currDirList[item].split("/")[0]].appWindow
													.appImg
											) +
											" " +
											this.currDirList[item].split("\\/").join("/") +
								...
								if (this.currDirList[item]) {
									...
									if (
										...
										temphtml +=
											...
											"')\" oncontextmenu=\"ctxMenu([[event.pageX, event.pageY, 'ctxMenu/file.png', 'ctxMenu/x.png'], ' Properties', 'apps.properties.main(\\'openFile\\', \\'" +
											(this.currLoc + this.currDirList[item]) +
											"\\');toTop(apps.properties)', '+Delete', 'apps.files.vars.deleteItem(\\'" +
											(this.currLoc + this.currDirList[item]) +
											"\\')'])\">" +
											'<img src="files/small/folder.png"> ' +
											this.currDirList[item].split("\\/").join("/") +
										...
										temphtml +=
											'<div class="cursorPointer" onClick="apps.notepad.vars.openFile(\'/window' +
											(this.currLoc + this.currDirList[item])
												.split("\\")
												.join("\\\\") +
											"');\" oncontextmenu=\"ctxMenu([[event.pageX, event.pageY, 'ctxMenu/file.png', 'ctxMenu/x.png'], ' Properties', 'apps.properties.main(\\'openFile\\', \\'" +
				...
				const pathSplit = this.currLoc.split("/");
				if (pathSplit[0] === "") {
					pathSplit.shift();
				}
					...
					for (var j = 0; j < pathSplit.length - 1; j++) {
						if (pathSplit[j][pathSplit[j].length - 1] === "\\") {
							pathSplit.splice(
								j,
								2,
								pathSplit[j].substring(0, pathSplit[j].length) +
									"/" +
									pathSplit[j + 1]
							);
							cleanEscapeRun = 0;
					...
					if (cleanEscapeRun && pathSplit.length > 0) {
						if (
							pathSplit[pathSplit.length - 1][
								pathSplit[pathSplit.length - 1].length - 1
							] === "\\"
						) {
							pathSplit.splice(
								j,
								1,
								pathSplit[pathSplit.length - 1].substring(
									0,
									pathSplit[pathSplit.length - 1].length
								) + "/"
							);
							cleanEscapeRun = 0;
						}
				...
				let navPath = "/";
				let tempHTML =
					"<div class=\"cursorPointer\" onclick=\"apps.files.vars.currLoc = '/';apps.files.vars.update()\" oncontextmenu=\"ctxMenu([[event.pageX, event.pageY, 'ctxMenu/file.png', 'ctxMenu/x.png'], '-Properties', '', '_Delete', ''])\">" +
					'<img src="files/small/folder.png"> ' +
					"/" +
					"</div>";
				for (const i in pathSplit) {
					if (pathSplit.indexOf("apps") === 0 && navDepth === 1) {
						tempHTML +=
							'<div class="cursorPointer" onclick="apps.files.vars.currLoc = \'' +
							navPath +
							"';apps.files.vars.next('" +
							pathSplit[i] +
							"/')\" oncontextmenu=\"ctxMenu([[event.pageX, event.pageY, 'ctxMenu/file.png', 'ctxMenu/x.png'], ' Properties', 'apps.properties.main(\\'openFile\\', \\'" +
							(navPath + pathSplit[i]) +
							"/\\');toTop(apps.properties)', '_Delete', ''])\">" +
			...
			updateSearch: function (searchStr) {
				const searchElems =
					getId("FIL2tbl").getElementsByClassName("cursorPointer");
				for (let i = 0; i < searchElems.length; i++) {
					if (
						searchElems[i].innerText
							.toLowerCase()
			...
			updateFavorites: function (nosave, mainPage) {
				...
				for (const i in this.favorites) {
					const currPath = this.favorites[i].split("/");
					let cleanEscapeRun = 0;
					while (!cleanEscapeRun) {
						cleanEscapeRun = 1;
						for (let j = 0; j < currPath.length - 1; j++) {
							if (currPath[j][currPath[j].length - 1] === "\\") {
								currPath.splice(
									j,
									2,
									currPath[j].substring(0, currPath[j].length) +
										"/" +
										currPath[j + 1]
								);
								cleanEscapeRun = 0;
						...
						if (cleanEscapeRun && currPath.length > 0) {
							if (
								currPath[currPath.length - 1][
									currPath[currPath.length - 1].length - 1
								] === "\\"
							) {
								currPath.splice(
									j,
									1,
									currPath[currPath.length - 1].substring(
										0,
										currPath[currPath.length - 1].length
									) + "/"
								);
								cleanEscapeRun = 0;
							}
					...
					if (currPath.length === 0) {
						tempHTML +=
							"<div class=\"cursorPointer\" onclick=\"apps.files.vars.currLoc = '/';apps.files.vars.update()\" oncontextmenu=\"ctxMenu([[event.pageX, event.pageY, 'ctxMenu/file.png', 'ctxMenu/x.png'], '-Properties', '', '+Remove Favorite', 'apps.files.vars.toggleFavorite(\\'/\\')', '_Delete', ''])\">" +
							'<img src="files/small/folder.png"> ' +
							"/" +
							"</div>";
					} else {
						const currName = currPath[currPath.length - 1];
						if (currPath.indexOf("apps") === 0 && currPath.length === 2) {
							tempHTML +=
								'<div class="cursorPointer" onclick="apps.files.vars.currLoc = \'' +
								this.favorites[i] +
								"';apps.files.vars.update()\" oncontextmenu=\"ctxMenu([[event.pageX, event.pageY, 'ctxMenu/file.png', 'ctxMenu/x.png'], ' Properties', 'apps.properties.main(\\'openFile\\', \\'" +
								this.favorites[i] +
								"\\');toTop(apps.properties)', '+Remove Favorite', 'apps.files.vars.toggleFavorite(\\'" +
								this.favorites[i] +
								"\\')', '_Delete', ''])\">" +
								buildSmartIcon(
									16,
			...
			toggleFavorite: function (item) {
				const itemLocation = this.favorites.indexOf(item);
				if (itemLocation === -1) {
			...
			switch (signal) {
				...
				case "USERFILES_DONE":
					...
					if (ufload("system/apps/files/favorites")) {
						this.vars.favorites = JSON.parse(
							ufload("system/apps/files/favorites")
						);
					}
					break;
				default:
