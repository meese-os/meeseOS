...
apps.help = new Application({
	...
	main: function() {
		...
		for (let appHandle in appsSorted) {
			...
			getId("helpDiv").innerHTML +=
				'<div id="APBapp_' + app + '" class="helpItem cursorPointer darkResponsive" onclick="c(function(){openapp(apps.' + app + ', \'dsktp\')});" ' +
				...
				']);">' +
					...
					'<div class="helpItemText">' +
						'<p class="helpAppName">' + this.vars.currAppName + '</p>' +
						'<p class="helpAppDesc">' + this.vars.currAppDesc + '</p>' +
					'</div>' +
				'</div>';
	...
	vars: {
		...
		search: function (text) {
			let allDivs = getId("helpDiv").getElementsByClassName('helpItem');
			let textSplit = text.toLowerCase().split(" ");
			for (let i = 0; i < allDivs.length; i++) {
				let isVisible = false;
				let texts = allDivs[i].getElementsByClassName("helpAppName");
