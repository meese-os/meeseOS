const Bash = () => {
	apps.bash = new Application({
		...
		vars: {
			appInfo:
				'This app is intended to imitate a Linux Bash Terminal, but is written completely in JavaScript. Type "help" for available commands and usage.',
							...
							if (
								...
								try {
									new Function(
										...
										"var " + this.workdirtemp[this.workdirdepth]
									);
									this.workdirfinal +=
										"." + this.workdirtemp[this.workdirdepth];
								} catch (err) {
				...
				if (this.piping) {
					...
					try {
						getId("bashContent").innerHTML +=
							"<br>" +
							cleanStr(String(message))
								.split("  ")
								.join(" &nbsp;")
								.split("\n")
								.join("<br>") +
							"&nbsp;";
						getId("win_bash_html").scrollTop =
							getId("win_bash_html").scrollHeight;
					} catch (err) {
			...
			getCmdObjects: function (command, alias) {
				...
				while (prev < command.length) {
					...
					if (i === -1 && j === -1 && s === -1) {
						...
						var postAlias = this.getAlias(
							command.substring(prev, command.length),
							alias
						);
					...
					if (s !== -1 && (s < i || i === -1) && (s < j || j === -1)) {
						...
						while (command[tempCurr - 1] === "\\") {
							command =
								command.substring(0, tempCurr - 1) +
								command.substring(tempCurr, command.length);
			...
			execute: function (cmd, silent) {
				if (cmd) {
					...
					if (silent) {
						...
						if (arguments.callee.caller === window.sh) {
							if (
								typeof arguments.callee.caller.caller.__bashworkdir === "string"
							) {
					...
					this.command = getId("bashInput").value.substring(
						getId("bashInput").value.indexOf("$") + 2,
						getId("bashInput").value.length
					);
					this.appendHistory(
						getId("bashInput").value.substring(
							getId("bashInput").value.indexOf("$") + 2,
							getId("bashInput").value.length
						)
					);
				...
				if (this.command.length !== 0) {
					...
					if (commandObjects.indexOf("|") !== -1) {
						for (
							var i = 0;
							commandObjects.indexOf("|", i) !== -1;
							i = commandObjects.indexOf("|", i) + 1
						) {
							var pipeGroup = [];
						...
						var pipeGroup = [];
						for (
							var j = commandObjects.lastIndexOf("|") + 1;
							j < commandObjects.length;
							j++
						) {
							pipeGroup.push(commandObjects[j]);
						}
					...
					for (var i = 0; i < pipeGroups.length; i++) {
						const currCmd = pipeGroups[i].shift();
						let cmdID = -1;
			...
			cmdHistory: [],
			currHistory: -1,
			seekHistory: function (direction) {
				// 1 or -1
				const nextHistory = this.currHistory + direction;
				if (nextHistory < 0) {
					this.currHistory = -1;
					return "";
			...
			commands: [
				{
					...
					action: function (args) {
						...
						this.vars.foundCmds = apps.bash.vars.commands.filter(function (i) {
							return (
								apps.bash.vars.currHelpSearch.indexOf(i.name) > -1 ||
								i.name.indexOf(apps.bash.vars.currHelpSearch) > -1
							);
						});
						let str = "";
						for (const i in this.vars.foundCmds) {
							str +=
								"\n\n" +
								this.vars.foundCmds[i].name +
								": " +
								this.vars.foundCmds[i].usage;
				...
				},
				{
					...
					action: function (args) {
						if (args.length > 0) {
							if (
								(args[0].length > 0 && args[1] === "=") ||
								args[0].length > 1
							) {
								if (args[0].indexOf("=") === args[0].length - 1) {
									const shifted = args.shift();
									apps.bash.vars.alias[
										shifted.substring(0, shifted.length - 1)
									] = args.join(" ");
								} else if (args[1] === "=") {
									const shifted = args.shift();
									args.shift();
							...
							ufsave(
								"system/apps/bash/alias",
								JSON.stringify(apps.bash.vars.alias)
							);
						} else {
							let str = "";
							for (const i in apps.bash.vars.alias) {
					...
					action: function (args) {
						if (args.length > 0) {
							if (args[0].toLowerCase() === "-j") {
								return (
									"shdir: " +
									apps.bash.vars.workdir +
									"\n" +
									"jsdir: " +
									apps.bash.vars.translateDir(apps.bash.vars.workdir)
								);
							} else {
							...
							try {
								...
								while (!cleanEscapeRun) {
									...
									for (var i = 0; i < this.vars.tempadd.length - 1; i++) {
										if (
											this.vars.tempadd[i][this.vars.tempadd[i].length - 1] ===
											"\\"
										) {
											this.vars.tempadd.splice(
												i,
												2,
												this.vars.tempadd[i].substring(
													0,
													this.vars.tempadd[i].length
												) +
													"/" +
													this.vars.tempadd[i + 1]
											);
											cleanEscapeRun = 0;
											break;
										}
									...
									if (cleanEscapeRun && this.vars.tempadd.length > 0) {
										if (
											this.vars.tempadd[this.vars.tempadd.length - 1][
												this.vars.tempadd[this.vars.tempadd.length - 1].length -
													1
											] === "\\"
										) {
											this.vars.tempadd.splice(
								...
								while (!cleanEscapeRun) {
									...
									for (let i = 0; i < this.vars.tempdir.length - 1; i++) {
										if (
											this.vars.tempdir[i][this.vars.tempdir[i].length - 1] ===
											"\\"
										) {
											this.vars.tempdir.splice(
												i,
												2,
												this.vars.tempdir[i].substring(
													0,
													this.vars.tempdir[i].length
												) +
													"/" +
													this.vars.tempdir[i + 1]
											);
											cleanEscapeRun = 0;
											break;
										}
									...
									if (cleanEscapeRun && this.vars.tempdir.length > 0) {
										if (
											this.vars.tempdir[this.vars.tempdir.length - 1][
												this.vars.tempdir[this.vars.tempdir.length - 1].length -
													1
											] === "\\"
										) {
											this.vars.tempdir.splice(
								...
								for (const i in this.vars.tempadd) {
								...
								if (apps.bash.vars.workdir === "/") {
									apps.bash.vars.prefix = `[${SRVRKEYWORD.substring(
										0,
										4
									)}@${websiteTitle} /]$ `;
								} else {
				...
				},
				{
					...
					action: function (args) {
						...
						this.vars.out = "";
						for (const i in this.vars.lines) {
							if (
								this.vars.lines[i]
									.toLowerCase()
									.indexOf(this.vars.target.toLowerCase()) > -1
							) {
								this.vars.out += "\n" + this.vars.lines[i];
							}
						}
					...
					action: function (args) {
						...
						if (typeof this.vars.selectedDir) {
							if (typeof this.vars.selectedDir === "object") {
								for (const item in this.vars.selectedDir) {
									dirSize++;
									if (dirSize > 1) {
										printBuffer += "\n" + item.split("/").join("\\/");
									} else {
									...
									if (typeof this.vars.selectedDir[item] === "object") {
										...
										if (this.vars.printSub) {
											for (const subitem in this.vars.selectedDir[item]) {
												printBuffer +=
													"\n" +
													item.split("/").join("\\/") +
													"/" +
													subitem.split("/").join("\\/");
												if (
													typeof this.vars.selectedDir[item][subitem] ===
													"object"
												) {
													printBuffer += "/";
												}
				...
				},
				{
					...
					action: function (args) {
						if (args.length > 1) {
							...
							eval(
								apps.bash.vars.translateDir(this.vars.currSet[1]) +
									"=" +
									apps.bash.vars.translateDir(this.vars.currSet[0])
							);
					...
					action: function (args) {
						if (args.length > 0) {
							for (const item in args) {
								...
								for (const i in this.vars.stack) {
									if (this.vars.first) {
									...
									if (
										typeof apps.bash.vars.getRealDir(this.vars.trace) !==
										"object"
									) {
										if (
											apps.bash.vars.getRealDir(this.vars.trace) === undefined
										) {
											eval(
												apps.bash.vars.translateDir(this.vars.trace) + " = {}"
											);
										} else {
				...
				},
				{
					...
					action: function (args) {
						const rand = Math.floor(Math.random() * this.vars.fortunes.length);
						return this.vars.fortunes[rand];
					},
					vars: {
						fortunes: ["Test Fortune 1", "Test Fortune 2", "Test Fortune 3"],
					},
				},
				{
					name: "exit",
				...
				},
				{
					name: "repo",
					usage:
						"repo {install [repo.]pkg | remove [repo.]pkg | update | upgrade | add-repo repo | remove-repo repo | search [query | repo | repo.query] | list | list-all | list-updates}",
						...
						if (args.length > 0) {
							...
							switch (currentAction) {
								...
								case "add-repo":
									...
									break;
								case "remove-repo":
									var result = repoRemoveRepository(
										args.trim(),
										apps.bash.vars.echo
									);
									if (typeof result !== "boolean") return result;
		...
		signalHandler: function (signal) {
			switch (signal) {
				...
				case "shrink":
					...
					break;
				case "USERFILES_DONE":
					this.vars.prefix = `[${SRVRKEYWORD.substring(
						0,
						4
					)}@${websiteTitle} bash]$ `;
					this.vars.pastValue = `[${SRVRKEYWORD.substring(
						0,
						4
					)}@${websiteTitle} bash]$ `;
