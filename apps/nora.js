const NORA = () => {
		...
		main: function (launchtype) {
			if (!this.appWindow.appIcon) {
				...
				this.appWindow.setContent(
					`<div id="NORAout" class="darkResponsive">-- ${websiteTitle} Assistant Ready --</div><button id="NORAspeech" onclick="apps.nora.vars.speakIn()">Speak</button><input id="NORAin" onKeydown="if(event.keyCode === 13){apps.nora.vars.input()}"><button id="NORAbtn" onClick="apps.nora.vars.input()">Say</button>`
				);
			...
			if (launchtype === "srtup") {
				if (window.webkitSpeechRecognition) {
					...
					this.vars.recognition.onresult = function (event) {
						...
						if (event.results[0].isFinal) {
							if (apps.nora.vars.currentlySpeaking) {
								...
								getId("NORAin").style.borderColor =
									"rgb(" +
									Math.round(255 - event.results[0][0].confidence * 255) +
									"," +
									Math.round(event.results[0][0].confidence * 255) +
									",0)";
							} else {
					...
					this.vars.contRecog.onresult = function (event) {
						...
						if (
							!apps.nora.vars.currentlySpeaking &&
							getId("NORAin").value === "" &&
							apps.nora.vars.currContTrans.indexOf(currNoraPhrase) > -1
						) {
				...
				try {
					apps.nora.vars.speakWordsMsg = new window.SpeechSynthesisUtterance(
						"test"
					);
					...
					window.speechSynthesis.onvoiceschanged = function () {
						...
						if (!apps.nora.vars.initing) {
							apps.nora.vars.speakWordsMsg.voice = apps.nora.vars.voices.filter(
								function (voice) {
			...
			commands: [
				...
				[
					...
					function (text) {
						if (text) {
							if (
								!isNaN(
									parseInt(text.split(" ")[text.split(" ").length - 1], 10)
								)
							) {
								if (
									parseInt(text.split(" ")[text.split(" ").length - 1], 10) >
									...
									this[4].lastDelete =
										...
										1;
									this[4].lastDeleted =
										apps.nora.vars.notes[this[4].lastDelete];
									for (
										let i = this[4].lastDelete;
										i < apps.nora.vars.notes.length - 1;
										i++
									) {
									...
									apps.nora.vars.say(
										"I can't delete something that's not there. You only have " +
											apps.nora.vars.notes.length +
											" notes."
									);
								}
							...
							apps.nora.vars.say(
								"<i>NORAA does just that - deletes nothing.</i>"
							);
						}
					},
					{
				...
				[
					...
					function (text) {
						if (text.toLowerCase().indexOf("and predict") === 0) {
							...
							apps.nora.vars.say(
								"I call " + this[4].coins[this[4].guess] + "!"
							);
						}
						this[4].result = Math.round(Math.random());
						apps.nora.vars.say(
							"I flipped " + this[4].coins[this[4].result] + "!"
						);
						if (text.toLowerCase().indexOf("and predict") === 0) {
				...
				[
					...
					function (text) {
						if (text) {
							this[4].cmdFound = -1;
							for (const cmd in apps.nora.vars.commands) {
								if (
									apps.nora.vars.commands[cmd][0] === text &&
									apps.nora.vars.commands[cmd][1] !== null
								) {
									this[4].cmdFound = cmd;
									break;
								}
							...
							if (this[4].cmdFound !== -1) {
								apps.nora.vars.say(
									'<span style="color:#F80">' +
										apps.nora.vars.commands[this[4].cmdFound][0] +
										"</span>"
								);
								apps.nora.vars.say(
									'<span style="color:#ACE">Usage: ' +
										apps.nora.vars.commands[this[4].cmdFound][1] +
										"</span>"
								);
								apps.nora.vars.say(
									apps.nora.vars.commands[this[4].cmdFound][2]
								);
							} else {
							...
							apps.nora.vars.say(
								"When speaking to me, the color of the button and the input field's border will give these indicators..."
							);
							apps.nora.vars.say(
								'<span style="color:#F00">Red Button</span> means that I am listening to you. If you messed up, you can click the button again to cancel.'
							);
							apps.nora.vars.say(
								'<span style="color:#0F0">Green Button</span> means that I am done listening and am giving you some time to cancel the spoken input, if needed, before accepting it.'
							);
							apps.nora.vars.say(
								'<span style="color:#F80">Orange Button</span> means that you have cancelled speech recognition. If I still seem to be listening, it\'s okay. I will not accept the input generated.'
							);
							apps.nora.vars.say(
								'The <span style="color:#0F0">greener</span> the border of the input is, the more confident I am that I heard you right. The <span style="color:#F00">redder</span> it is, the less confident.'
							);
							apps.nora.vars.say(
								"List of all NORAA commands... Args key: [required arg] {optional arg}"
							);
							for (const command in apps.nora.vars.commands) {
								if (apps.nora.vars.commands[command][1] !== null) {
									apps.nora.vars.say(
										'<span style="color:#F80">' +
											apps.nora.vars.commands[command][0] +
											"</span>"
									);
									apps.nora.vars.say(
										'<span style="color:#ACE">Usage: ' +
											apps.nora.vars.commands[command][1] +
											"</span>"
									);
				...
				[
					...
					function (text) {
						if (this[4].phrases[text.toLowerCase()]) {
							...
							apps.nora.vars.say(
								"Sorry, I do not know how to " + text + " at the moment."
							);
						}
					},
					{
						...
						phrases: {
							"open apps":
								"To open apps, you can click the app's desktop icon, or if the app has been minimised, click its icon on the taskbar. Alternatively, all apps will appear in the applications list in the bottom-left corner of the screen.",
							"talk to you":
								'Your computer must support the speech engine. If you cannot see the "speak" button to the left of my input box, that means your computer doesn\'t.',
							"move desktop icons":
								'You may right-click the icon, then click "Move Icon", then click some location on the desktop.',
							"move windows":
								"You can click on the top title bar of the window, then click somewhere else.",
							"resize windows":
								"You can click the bottom half of the border of the window, then click somewhere else.",
						},
					},
				],
				[
				...
				[
					...
					function (text) {
						if (text.indexOf(" is ") > -1) {
							...
							apps.nora.vars.say(
								"Thank you for telling me that your " +
									this[4].inpPro +
									" is " +
									this[4].inpVal
							);
							...
							delete apps.nora.vars.userObj[
								text.substring(0, text.indexOf(" will be deleted"))
							];
							ufsave(
								"system/noraa/user_profile",
								JSON.stringify(apps.nora.vars.userObj)
							);
							apps.nora.vars.say("I deleted that info about you.");
						} else {
							apps.nora.vars.say(
								"I cannot find any discernable information in there."
							);
						}
					},
					{
				...
				[
					...
					function (text) {
						if (this[4].colors.hasOwnProperty(text.toLowerCase())) {
							...
							for (const i in this[4].colors._COLORS) {
							...
							apps.nora.vars.say(
								"<span style='color:rgb(" +
									this[4].colors._COLORS.r +
									"," +
									this[4].colors._COLORS.g +
									"," +
									this[4].colors._COLORS.b +
									")'>Here's a random shade of " +
									text.toLowerCase() +
									"!</span>"
							);
							apps.nora.vars.say(
							...
							apps.nora.vars.say(
								"I don't know what that color is, sorry! Make sure also that you arent using punctuation!"
							);
						}
					},
					{
						colors: {
							...
							red: function () {
								...
								this._COLORS.g = Math.floor(
									Math.random() * (this._COLORS.r / 3)
								);
								this._COLORS.b = Math.floor(
									Math.random() * (this._COLORS.r / 3)
								);
							},
							green: function () {
								this._COLORS.g = Math.floor(Math.random() * 256);
								this._COLORS.r = Math.floor(
									Math.random() * (this._COLORS.g / 3)
								);
								this._COLORS.b = Math.floor(
									Math.random() * (this._COLORS.g / 3)
								);
							...
							yellow: function () {
								this._COLORS.r = Math.floor(Math.random() * 256);
								this._COLORS.g =
									this._COLORS.r + (Math.floor(Math.random() * 30) - 15);
								this._COLORS.b = Math.floor(
									Math.random() * ((this._COLORS.r + this._COLORS.g) / 2 / 3)
								);
							},
							teal: function () {
								this._COLORS.b = Math.floor(Math.random() * 256);
								this._COLORS.g =
									this._COLORS.b + (Math.floor(Math.random() * 30) - 15);
								this._COLORS.r = Math.floor(
									Math.random() * ((this._COLORS.b + this._COLORS.g) / 2 / 3)
								);
							},
							violet: function () {
								this._COLORS.r = Math.floor(Math.random() * 256);
								this._COLORS.b =
									this._COLORS.r + (Math.floor(Math.random() * 30) - 15);
								this._COLORS.g = Math.floor(
									Math.random() * ((this._COLORS.r + this._COLORS.b) / 2 / 3)
								);
							},
							...
							orange: function () {
								this._COLORS.r = Math.floor(Math.random() * 256);
								this._COLORS.g = Math.floor(
									Math.random() * (this._COLORS.r * 0.7) + this._COLORS.r * 0.15
								);
								this._COLORS.b = Math.floor(
									Math.random() * ((this._COLORS.r + this._COLORS.g) / 2 / 3)
								);
							...
							turquoise: function () {
								this._COLORS.g = Math.floor(Math.random() * 256);
								this._COLORS.b = Math.floor(
									Math.random() * (this._COLORS.g * 0.7) + this._COLORS.g * 0.15
								);
								this._COLORS.r = Math.floor(
									Math.random() * ((this._COLORS.g + this._COLORS.b) / 2 / 3)
								);
							},
							cyan: function () {
								this._COLORS.b = Math.floor(Math.random() * 256);
								this._COLORS.g = Math.floor(
									Math.random() * (this._COLORS.b * 0.7) + this._COLORS.b * 0.15
								);
								this._COLORS.r = Math.floor(
									Math.random() * ((this._COLORS.b + this._COLORS.g) / 2 / 3)
								);
							...
							purple: function () {
								...
								this._COLORS.g = Math.floor(
									Math.random() * ((this._COLORS.b + this._COLORS.r) / 2 / 3)
								);
							},
						},
					},
				...
				[
					...
					function (text) {
						if (apps.nora.vars.notes.length !== 0) {
							apps.nora.vars.sayDynamic("okay");
							apps.nora.vars.say(
								"If you need a note deleted, just ask me to delete that note number."
							);
							for (const i in apps.nora.vars.notes) {
								apps.nora.vars.say(
									'<span style="color:#F80">' +
										(parseInt(i, 10) + 1) +
										": </span>" +
										apps.nora.vars.notes[i]
								);
							}
						} else {
							apps.nora.vars.say(
								"<i>NORAA does as you say - reads all 0 of your notes.</i>"
							);
						}
					},
				],
						...
						if (text) {
							if (text.indexOf("out loud") === 0) {
								if (!apps.nora.vars.lastSpoken) {
									apps.nora.vars.waitingToSpeak.push(
				...
				[
					...
					function (text) {
						if (text) {
							...
							apps.nora.vars.say(
								"<i>NORAA does as you asked - takes note of nothing</i>"
							);
						}
					},
				],
						...
						if (text.length > 0) {
							switch (text.toLowerCase()) {
								...
								case "your favorite color":
									this[4].lastColor = Math.floor(
										Math.random() * this[4].colors.length
									);
									apps.nora.vars.say(
										'<span style="color:' +
											this[4].colors[this[4].lastColor][1] +
											";\">Right now, I'm feeling " +
											this[4].colors[this[4].lastColor][0] +
											"."
									);
									break;
								default:
							...
							apps.nora.vars.say(
								"I don't know what nothing is, but I won't bother searching DuckDuckGo for it. All they would give you is something, which is not what you seem to be looking for."
							);
						}
					},
					{
						...
						colors: [
							...
							["cyan", "#0FF"],
							[
								'blue... </span><span style="color:#44B">n</span><span style="color:#999">o</span><span style="color:#BB4">,</span><span style="color:#FF0"> yel- aaAAAAAAAAHHHHhhhhh..',
								"#00F",
							],
						],
					},
				...
				[
					...
					function () {
						...
						for (const i in apps.nora.vars.userObj) {
							apps.nora.vars.say(
								"Your " + i + " is " + apps.nora.vars.userObj[i]
							);
						}
							...
							if (text.toLowerCase().indexOf("yes") > -1) {
								apps.nora.vars.specialCommand = function (text) {
									if (text.toLowerCase().indexOf("my") === 0) {
										...
										apps.nora.vars.say(
											"Not really sure what information I can glean from that."
										);
									}
								};
								apps.nora.vars.say(
									'What would you like to change? (say "my [whatever] is [something]")'
								);
							} else {
				...
				[
					...
					function (text) {
						if (text.length > 0) {
							...
							apps.nora.vars.say(
								"I don't know who nobody is, but I won't bother searching DuckDuckGo for them. All they would give you is something, which is not what you seem to be looking for."
							);
						}
					},
					{
			...
			input: function (spoken, silent) {
				...
				if (this.specialCommand === null) {
					for (const cmd in this.commands) {
						if (
							this.lastIn.toLowerCase().indexOf(this.commands[cmd][0]) === 0
						) {
							this.inSuccess = 1;
							this.commands[cmd][3](
								this.lastIn.substring(
									this.commands[cmd][0].length + 1,
									this.lastIn.length
								)
							);
							break;
			...
			switch (signal) {
				...
				case "USERFILES_DONE":
					...
					if (ufload("system/noraa/speech_response_delay")) {
						this.vars.inputDelay = parseInt(
							ufload("system/noraa/speech_response_delay"),
							10
						);
					}
	...
	function textspeech(message) {
		...
		apps.nora.vars.say(
			'<span style="color:#ACE">Text-to-speech from selection:</span>'
		);
