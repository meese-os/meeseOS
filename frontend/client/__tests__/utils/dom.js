...
describe("DOM Utils", () => {
	test("escapeHtml", () => {
		expect(escapeHtml("This is a <div>test</div>")).toBe("This is a test");
	});
	...
	test("script - onload", () => {
		return expect(script(document.body, "onload")).resolves.toBeInstanceOf(
			HTMLScriptElement
		);
	});
	...
	test("script - onreadystatechange", () => {
		return expect(
			script(document.body, "onreadystatechange")
		).resolves.toBeInstanceOf(HTMLScriptElement);
	});

	test("script - onerror", () => {
		return expect(script(document.body, "onerror")).rejects.toBeInstanceOf(
			Error
		);
	});

	test("style - onload", () => {
		return expect(style(document.body, "onload")).resolves.toBeInstanceOf(
			HTMLLinkElement
		);
	});

	test("style - onerror", () => {
		return expect(style(document.body, "onerror")).rejects.toBeInstanceOf(
			Error
		);
	});
});
