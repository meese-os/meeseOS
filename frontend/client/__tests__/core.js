...
describe("Core", () => {
	...
	test("#off - fail unregister event", () => {
		return expect(() => core.off("meeseOS/fail")).toThrow(TypeError);
	});

	test("#off - unregister event", () => {
		return expect(() => core.off("meeseOS/fail", () => {})).not.toThrow(
			TypeError
		);
	});

	test("#boot", () => {
		return expect(core.boot()).resolves.toBe(false);
	});

	test("#start", () => {
		return expect(core.start()).resolves.toBe(false);
	});

	test("#url", () => {
		expect(core.url("foo/bar")).toBe("/foo/bar");
		expect(core.url("foo/bar", { prefix: true })).toBe(
			"http://localhost/foo/bar"
		);
		expect(core.url("foo/bar", { prefix: true, type: "websocket" })).toBe(
	...
	test("#request - standalone error", () => {
		...
		return expect(c.request("/foo")).rejects.toThrow();
	});

	test("#request", () => {
		return expect(
			core.request("/success-test").then((response) => response.json())
		).resolves.toBe(true);
	});

	test("#request - handle failure", () => {
		return expect(
			core.request("/fail-test").then((response) => response.json())
		).rejects.toThrow();
	});

	test("#run - fail to run unknown application", () => {
		return expect(() => core.run("UnknownApplication")).toThrow();
	});

	test("#run - run valid application", () => {
		return expect(() => core.run("ValidApplication")).not.toThrow();
	});
