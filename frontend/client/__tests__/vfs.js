...
describe("VFS", () => {
	...
	test("#readfile - string", () => {
		return expect(call("readfile", "null:/filename")).resolves.toBe("");
	});

	test("#readfile - blob", () => {
		return expect(
			call("readfile", "null:/filename", "blob")
		).resolves.toBeInstanceOf(Blob);
	});

	test("#readfile - uri", () => {
		return expect(call("readfile", "null:/filename", "uri")).resolves.toBe(
	...
	test("#readfile - arraybuffer", () => {
		return expect(
			call("readfile", "null:/filename", "arraybuffer")
	...
	test("#writefile - arraybuffer", () => {
		return expect(
			call("writefile", "null:/filename", new ArrayBuffer())
		).resolves.toBe(-1);
	});

	test("#writefile - string", () => {
		return expect(call("writefile", "null:/filename", "data")).resolves.toBe(
			-1
		);
	});

	test("#copy", () => {
		return expect(call("copy", "null:/from", "null:/to")).resolves.toBe(false);
	});

	test("#rename", () => {
		return expect(call("rename", "null:/from", "null:/to")).resolves.toBe(
			false
		);
	});

	test("#mkdir", () => {
		return expect(call("mkdir", "null:/directory")).resolves.toBe(false);
	});
	...
	test("#exists", () => {
		return expect(call("exists", "null:/filename")).resolves.toBe(false);
	});

	test("#url", () => {
		return expect(call("url", "null:/filename")).resolves.toBe(null); // Except that it doesn't
	});

	test("#search", () => {
		return expect(call("search", "null:/")).resolves.toBeInstanceOf(Array);
	});

	test("#search - not enabled", () => {
		return expect(callOther("search", "other:/")).resolves.toBeInstanceOf(
			Array
		);
	});
