...
export function createLettercrap() {
	const lettercrapTextElement = document.querySelector("[data-lettercrap-text]");
...
function render(element, image, prev) {
	...
	const words = element.hasAttribute("data-lettercrap-words") ? element.getAttribute("data-lettercrap-words").split(" ") : [];
	const letters = element.hasAttribute("data-lettercrap-letters") ? element.getAttribute("data-lettercrap-letters") : "0101010101_";
	const textCondition = prev && prev.width === element.clientWidth && prev.height === element.clientHeight;
	const text = getTextContentWithImageAtSize(image, element.clientWidth, element.clientHeight,
		textCondition ? prev.text : null, words, letters);

	element.textContent = text;
	const data = {width: element.clientWidth, height: element.clientHeight, text: text};
